...
export const showAPI = {
  ...
  getTicketsByShow: async (show_id, show_date_id) => {
    try {
      return await apiService.post(
        `${API_CONFIG.ENDPOINTS.SHOW_TICKETS}`,
        {
          show_id,
          show_date_id,
        },
  ...
  getShowDetailsByDateId: async (show_date_id) => {
    try {
      return await apiService.get(
        `${API_CONFIG.ENDPOINTS.SHOW_DETAILS_BY_DATE_ID}/${show_date_id}`,
        {
          includeAuth: true,
        }
      );
  ...
  getGuestListByShow: async (show) => {
    try {
      return await apiService.post(
        `${API_CONFIG.ENDPOINTS.GUEST_LIST_BY_SHOW}`,
        {
          show_id: show.id,
...
export const scanAPI = {
  ...
  scanTicket: async (show_id, show_date_id, ticket_code) => {
    try {
      return await apiService.post(
        API_CONFIG.ENDPOINTS.SCAN_TICKET,
        {
          show_id,
          show_date_id,
          ticket_code,
